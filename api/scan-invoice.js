...
RÈGLE STRICTE — SI PLUSIEURS PRIX UNITAIRES APPARAISSENT SUR LA MÊME LIGNE, PRENDS TOUJOURS LE PRIX NET (APRÈS REMISE), JAMAIS LE PRIX BRUT :
Certaines factures grossistes (fréquent chez les fournisseurs de boissons) affichent PLUSIEURS chiffres de prix unitaire sur la même ligne, dans cet ordre : un prix brut ("P.U.HTT", "Prix catalogue", "Prix brut" — AVANT remise), puis une ligne "Remise"/"Rem."/"Ristourne" (souvent en pourcentage ou en valeur négative), puis un "Prix net" (ou "Prix net HT", ou juste "Net" — APRÈS remise). C'est TOUJOURS ce dernier chiffre, le "Prix net" après remise, qui est le vrai prix payé pour ce produit — c'est lui qu'il faut recopier dans printedUnitPriceHT, jamais le premier prix brut/catalogue. Avant de répondre, pour chaque ligne qui affiche plusieurs chiffres de prix unitaire : repère s'il y a un mot "Remise"/"Rem."/"Ristourne" entre deux d'entre eux — si oui, prends TOUJOURS le chiffre situé APRÈS ce mot, jamais celui d'avant, même s'il est affiché en plus gros ou en premier sur la ligne.
Exemple concret : une ligne affiche "P.U.HTT 5,2221 — Base 1,000 — Remise -1,325 — Prix net 3,8970" → printedUnitPriceHT: 3.897 (le "Prix net", le dernier chiffre après la remise), JAMAIS 5.2221 (le prix catalogue avant remise, même s'il apparaît en premier). Se tromper ici gonfle le prix importé d'environ le pourcentage de la remise, sans que rien ne le signale ensuite — sois particulièrement rigoureux sur ce point.

ERREUR FRÉQUENTE À NE JAMAIS COMMETTRE — NE DIVISE JAMAIS UN PRIX DÉJÀ AU KG/L PAR LA QUANTITÉ :
Sur une facture avec une colonne "Prix U. HT" (ou équivalent) DÉJÀ exprimée au kg/L et une colonne "Total HT" séparée, ces deux colonnes sont indépendantes — le total est déjà égal à (quantité × prix unitaire), c'est un calcul FAIT PAR LE FOURNISSEUR, pas un calcul qu'il te reste à faire à l'envers. NE calcule JAMAIS toi-même "Total HT ÷ Quantité" pour "retrouver" un prix unitaire que tu as pourtant sous les yeux, déjà imprimé — copie directement le chiffre de la colonne prix unitaire, un point c'est tout. Exemple concret d'erreur à éviter : une ligne "Bœuf — 5 kg — Prix U. HT 16,00€ — Total HT 80,00€" doit donner printedUnitPriceHT: 16 (le chiffre imprimé dans la colonne prix unitaire), JAMAIS 3,20 (= 80÷5, un calcul que tu n'avais pas à faire et qui est faux ici). AVANT DE RÉPONDRE, pour chaque ligne où printedPriceUnit vaut "kg" ou "L" : vérifie mentalement que printedUnitPriceHT × la quantité totale imprimée sur la ligne redonne bien le totalPriceHT imprimé (à l'arrondi près) — si ce n'est pas le cas, c'est le signe que tu as recopié le mauvais chiffre (peut-être confondu avec un calcul), reviens en arrière et recopie le vrai chiffre de la colonne prix unitaire, jamais une valeur que tu as calculée toi-même.
