...
- Scanner de factures, 2 corrections supplémentaires (2026-07-30) — bug réel observé : une ligne "prix par colis" sans poids indiqué (ex: frites surgelées vendues au colis sans mention de poids) ressortait à un prix aberrant (prix du colis affiché tel quel en €/kg) au lieu d'être bloquée :
  ...
  2. (⚠️ routage revu le 2026-07-30, voir juste en dessous) Nouveau champ `priceUnusable` (prix introuvable/incalculable) introduit pour empêcher tout import silencieux à 0€.
...
Aucun test réel effectué sur les 3 sessions de corrections du scanner : Node.js indisponible (ni chez l'utilisateur, ni dans l'environnement de dev), donc à valider en conditions réelles une fois déployé sur Vercel : rescanner une facture avec un produit non-alimentaire, une consigne, et un ingrédient alimentaire à prix illisible, pour confirmer que seuls les deux premiers finissent en "Ignorés" et que le troisième reste bien en "À vérifier".
