...
- **Carte digitale publique** (`api/public-menu.js`, `src/PublicMenu.jsx`, `src/MenuWizard.jsx`) : QR code, 4 designs, sections réordonnables, articles simples sans recette (`simpleItems`, avec conversion en recette via "Connaître sa marge", description+traduction depuis 2026-08-30), traduction auto FR/ES/EN, prix synchronisé en direct. **Réglages accessibles avant publication** (2026-08-30, corrigé — étaient invisibles tant que non publié) + aperçu "Voir la carte" fonctionnel même non publiée (`previewToken`, vérifié serveur contre le propriétaire du compte). Ajout rapide d'article utilise une pastille de section active (comme l'assistant guidé) pour ne plus jamais créer un article "sans section" par erreur.
