...
- Scanner de factures, 2 corrections supplémentaires (2026-07-30) — bug réel observé : une ligne "prix par colis" sans poids indiqué (ex: frites surgelées vendues au colis sans mention de poids) ressortait à un prix aberrant (prix du colis affiché tel quel en €/kg) au lieu d'être bloquée :
  ...
  2. Filtrage automatique vers "Ignorés" : nouveau champ `priceUnusable` (vrai si le prix est introuvable/incalculable, en plus des articles non-alimentaires déjà exclus). La section auparavant réservée aux non-alimentaires (`nonFoodItems`) est généralisée en `excludedItems`, couvrant les deux raisons (même UI, même bouton de récupération, tooltip qui précise la raison). Plus aucune ligne à prix inexploitable ne peut atterrir en "Sûr" à 0€, ni être importée via "Importer tout" même si l'utilisateur la récupère manuellement en vérification.

## EN COURS — prochaine étape demandée par l'utilisateur (pas encore codée)
_Rien en attente pour l'instant._ Aucun test réel effectué : Node.js indisponible (ni chez l'utilisateur, ni dans l'environnement de dev), donc à valider en conditions réelles une fois déployé sur Vercel : scanner une vraie facture avec au moins une ligne "prix au colis sans poids indiqué" (type frites surgelées) pour confirmer qu'elle atterrit bien en "Ignorés" plutôt qu'à un prix aberrant, et que "À vérifier"/la mémoire des rapprochements fonctionnent toujours normalement à côté.
