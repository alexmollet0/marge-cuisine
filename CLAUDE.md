...
- **Carte digitale publique** (`api/public-menu.js`, `src/PublicMenu.jsx`, `src/MenuWizard.jsx`) : QR code, 4 designs, sections réordonnables, articles simples sans recette (`simpleItems`, avec conversion en recette via "Connaître sa marge"), traduction auto FR/ES/EN, prix synchronisé en direct.
...
- **Tableau de bord admin** (`api/admin-dashboard.js`, visible `alexmollet0@gmail.com` seul) : onglet Comptes (cartes triées par activité, statut "en ligne" temps réel, chronologie par compte) et Aperçu (KPI, provenance de trafic par source, graphique visites/jour). Comptes internes exclus des stats.
...
- **Identité visuelle actuelle** : dégradé violet→cyan (`BRAND_GRADIENT`, `#7C3AED`→`#22D3EE`), fond ardoise chaud (`#1B1815`/`#26221C`), police Manrope, logo hirondelle (`src/Logo.jsx`). **⚠️ Refonte esthétique complète prévue en chat séparé** — voir EN COURS.
...
12. ☐ **Supprimer tous les comptes de test et leurs données** — demandé le 2026-08-28, pas encore fait. Comptes connus : `alexmollet0@gmail.com`, `contact.ttra@gmail.com` (déjà exclus des KPI mais pas supprimés), `contactchefup.app@gmail.com`, probablement `otptest12345@example.com`. **⚠️ Action destructrice — confirmer précisément avec l'utilisateur avant d'exécuter.**
